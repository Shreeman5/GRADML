...
def unique_elements(train, column):
    unique_list = []

    for x in train.loc[:, column]:
        if x not in unique_list and x != 'unknown':
            unique_list.append(x)

    dict_val = {}
    for x in unique_list:
        value_len = len(train.loc[:, column].loc[train.loc[:, column] == x])
        dict_val.update({x: value_len})

    return dict_val


def change_categorical_att(train, column):
    dict_val = unique_elements(train, column)
    majority = max(dict_val, key=dict_val.get)

    for x in range(len(train.loc[:, column])):
        if train.loc[x, column] == 'unknown':
            train.iloc[x, column] = majority
...
def training_part():
    ...
    column_1 = change_categorical_att(train, 1)

    column_3 = change_categorical_att(train, 3)

    median_val = find_median(train, 5)
    median_vals_for_test.append(median_val)
    column_5 = changed_column(train, 5, median_val)

    column_8 = change_categorical_att(train, 8)
    ...
    column_15 = change_categorical_att(train, 15)
    ...
    columns = pd.DataFrame(
        {'age': ['38.0+', '38.0-', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan'],
         'job': ['admin.', 'unemployed', 'management', 'housemaid', 'entrepreneur', 'student', 'blue-collar', 'self-employed', 'retired', 'technician', 'services', 'Nan'],
         'marital_status': ['married', 'divorced', 'single', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan'],
         'education': ['secondary', 'primary', 'tertiary', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan'],
         ...
         'contact': ['telephone', 'cellular', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan'],
         ...
         'poutcome': ['other', 'failure', 'success', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan', 'Nan']
         })
    # print(columns)
    #
...
def testing_part(trees, median_vals):
    ...
    test.loc[:, 1] = change_categorical_att(test, 1)
    test.loc[:, 3] = change_categorical_att(test, 3)
    test.loc[:, 5] = changed_column(test, 5, median_vals[1])
    test.loc[:, 8] = change_categorical_att(test, 8)
    ...
    test.loc[:, 15] = change_categorical_att(test, 15)
